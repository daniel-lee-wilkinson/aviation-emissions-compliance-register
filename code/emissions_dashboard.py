...
def update_dashboard(selected_operator, start_date, end_date):
    try:
        ...
        kpi_eea = f"{df_filtered['in_eea'].mean() * 100:.1f}%" if not df_filtered.empty else "0.0%"
        kpi_verified = f"{df_filtered['verified'].mean() * 100:.1f}%" if not df_filtered.empty else "0.0%"
