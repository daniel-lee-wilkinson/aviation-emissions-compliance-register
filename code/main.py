...
try:
    ...
    aircraft_types = {"A320": 2500, "A350": 5800, "B737": 2400, "B787": 5200}
...
df["in_eea"] = df.apply(
    lambda row: row["origin_country"] in eea_countries
    and row["destination_country"] in eea_countries,
    axis=1,
)
...
df["operator_compliant"] = df.apply(
    lambda row: row["verified"] and (np.random.rand() > 0.05), axis=1
)
...
summary = {
    ...
    "offset_required_tonnes": df.loc[df["offset_required"], "co2_emissions_kg"].sum()
    / 1000,
}
...
print(
    df[
        ["operator_name", "co2_emissions_kg", "in_eea", "verified", "offset_required"]
    ].head()
)
